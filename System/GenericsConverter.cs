...
namespace System
{
    public static unsafe class GenericsConverter
    {
        ...
        public static byte[] StructureToBytes<T>(T structure) where T : unmanaged
        {
            int length = Marshal.SizeOf<T>();
        ...
        public static T BytesToStructure<T>(ref byte[] bytes) where T : unmanaged
        {
            if (bytes == null)
            {
        ...
        public static T BytesToManaged<T>(ref byte[] bytes)
        {
            if (bytes == null)
            {
                throw new ArgumentNullException("bytes");
